...
def save_data_to_csv(symbol):
    web.DataReader(symbol, 'google', start, end).to_csv('data/' + symbol + '.csv')
...
def read_data(symbol):
    try:
        return pd.read_csv(symbol_to_path(symbol), index_col='Date', parse_dates=True, usecols=['Date', 'Adj Close'],
                           na_values=['nan'])
    except IOError:
...
def optimize(symbols, start_val, dates=dates):
    ...
    return minimize(port_objective, x0=allocs, args=(normed, start_val), method='SLSQP',
                    bounds=[(0, 1) for x in symbols], constraints=constraints)
